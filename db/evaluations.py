...
#read hashes with 53 indicators and pagespeed
    def getResultstoClassify(cursor, indicators):
        ...
        sql = "SELECT results_hash, results_url, results_main, sources_speed FROM results TABLESAMPLE SYSTEM_ROWS(2000) left join classifications on results_hash = classifications_hash JOIN sources ON sources_hash = results_hash JOIN evaluations ON results_hash = evaluations_results_hash WHERE classifications_hash IS NULL AND sources_speed IS NOT NULL GROUP BY 1,2,3,4 HAVING COUNT(evaluations_module) >= %s"
        data = (indicators)
        cursor.execute(sql,(data,))
        rows = cursor.fetchall()
        return rows
